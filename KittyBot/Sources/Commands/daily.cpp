  ...
  try
  {
    ...
    if (res.empty()) {
      dpp::embed cannot = dpp::embed()
        .set_title("Hold up!")
        .set_description("You've already claimed your prize for the day! Come back tomorrow.")
      ...
      kuser.lastclaimed = oss.str();

      trans.exec("UPDATE guildmember SET lastclaimed = $1 WHERE memberid = $2 AND guildid = $3;", pqxx::params {kuser.lastclaimed, uid, gid});
